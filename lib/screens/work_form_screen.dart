...
class _WorkFormScreenState extends ConsumerState<WorkFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      isEditing
          ? await listNotifier.updateWork(work.copyWith(id: widget.work!.id))
          : await listNotifier.addWork(work);
    } finally {
      if (mounted) {
  ...
  void initState() {
    ...
    if (widget.work != null) {
      ...
      _seasonOrChapterController.text = work.isReading
          ? work.chapter.toString()
          : work.season.toString();

      _episodeOrPageController.text = work.isReading
          ? work.page.toString()
